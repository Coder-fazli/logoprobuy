...
function SocialIcon({ href, label, iconKey }: { href?: string; label: string; iconKey: string }) {
  ...
  return (
    <a
      ...
      style={{
        color: hovered ? '#000000' : 'rgba(0,0,0,0.35)',
...
export default function SocialSidebar({ instagram, twitter, linkedin, youtube, dribbble }: Props) {
  const urls = { twitter, instagram, linkedin, youtube, dribbble };

  return (
    <div className="fixed left-10 top-1/2 -translate-y-1/2 z-40 hidden lg:flex flex-col items-center gap-1">
      <div className="w-px h-12 bg-border" />
      <div className="flex flex-col gap-6 py-4">
        {ALL_LINKS.map(({ key, label }) => (
          <SocialIcon key={key} href={urls[key]} label={label} iconKey={key} />
        ))}
      </div>
      <div className="w-px h-12 bg-border" />
    </div>
